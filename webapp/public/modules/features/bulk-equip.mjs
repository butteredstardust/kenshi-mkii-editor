...
import { addItemResults, raceFitWarnings } from '../items.mjs';
import { render, refresh } from '../nav.mjs';
import { buildRoster } from './roster.mjs';

/*
 * Bulk equip — moved here from gear.mjs (previously the Gear tab's own
 * multi-select panel) so the Loadouts tab, which is where a kit's contents are
 * actually named and browsed, is also where you apply one to a squad. This
 * file owns the roster multi-select mechanics too, because nothing else in
 * the app needs a checkbox roster once this panel isn't on Gear.
 *
 ...
 */
/**
 * Loadouts grouped for the <select>, using the SAME `categoryLabel` /
 * `categoryIndex` headings the catalogue above this panel is grouped by — one
 * page must not group the same 148 kits two different ways. This used to group
 * by the leading `tags` entry instead, which was fine at 66 kits but now puts
 * 52 of them under "Light armour" and leaves 8 under "Other".
 *
 * Exported for squad.mjs's "Add member" gear override select — same 148 kits,
 * same grouping, so a kit sits under the same heading on both panels.
 */
export function loadoutGroups() {
  const byIndex = new Map();
  for (const l of state.loadouts || []) {
    if (!byIndex.has(l.categoryIndex)) byIndex.set(l.categoryIndex, [l.categoryLabel, []]);
    byIndex.get(l.categoryIndex)[1].push(l);
  }
  return [...byIndex.entries()].sort(([a], [b]) => a - b).map(([, g]) => {
    g[1].sort((a, b) => a.label.localeCompare(b.label));
    return g;
  });
}

export function bulkPanel(picked) {
 ...
 * exists to avoid. It is the same picker as the per-character add-item control
 * (`items.mjs`), deliberately: one item search in this app, learned once. Ids
 * rather than `.add-item-*` classes because wire()'s per-card loop wires those
 * against a single character, and this card has no single character.
 */
...
/**
 * Multi-select roster + bulk equip (Loadouts tab).
 *
...
export function wireBulkEquip() {
   ...
   * the 0<->1 transition genuinely changes the layout (empty-state vs. bulk
   * panel); every other tick just updates a count, a heading and the
