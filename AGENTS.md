...
| GET | `/api/loadouts` | **37 named gear sets** for bulk equip (`services/loadouts.js`) — editorial, read off the game's own NPCs. Items already resolved to names/kinds, plus `tags` (heavy/light/ranged/support/trade/travel/starter) for grouping, advisory `raceNotes`, and a `missing[]` of any template this install cannot resolve |
