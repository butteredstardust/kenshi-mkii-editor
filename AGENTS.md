...
| `services/loadouts.js` | Named gear sets — 148 entries, each in one of 11 ordered `CATEGORIES`, backing both the Loadouts tab and bulk equip. Editorial, like `archetypes.js`; the `meitou` half is derived from gamedata character templates, not from a save. A missing or unknown `category` fails `validate()` |
...
| `services/colorsService.js` | The type-55 colour-scheme catalogue, resolved in load order: name, `color 1`/`color 2` as `#RRGGBB` swatches, and a type-3 armour template's own `extra['color']` allow-list (advisory only — never blocks a write). Disk-cached |
...
| `services/recruits.js` | "Roll a recruit" catalogue: 144 entries in 11 groups. Editorial, but races/tiers/weapon classes of the named ones come from the game's type-1 character records. `ranger` is the one group with **no** derived entries — not one of this install's 448 character templates carries a ranged weapon |
| `services/provisioning.js` | What a brand-new recruit arrives wearing and carrying: resolves a default loadout from archetype/sub/tier (a `recruits.js` entry's own `loadoutId` always wins), overrides its armour to Specialist (level 80) and its melee weapons to Catun No.3 — a grade decision separate from which loadout pieces are chosen — layers on a race-aware medical kit/food (a robotics kit and no food for a Skeleton), and rolls a random 300-5000 cats stack. Editorial like `loadouts.js`/`recruits.js`; `saveService.addSquadMember()` is the only caller that mints its output into a save |
...
| File | Role |
|---|---|
| `modules/system/shell.mjs` | `boot`, `render`, `refresh`, `wire`, `savePicker`, the tab buttons. `render()` dispatches on `state.current` to the feature renderers; `wire()` owns the per-card loop and calls each feature's own `wire*()` |
...
| GET | `/api/loadouts` | **148 named gear sets** (`services/loadouts.js`) — editorial, backing both the Loadouts tab and bulk equip. The first 37 were read off the game's own NPCs in a live save; the ones tagged `meitou` are the named Meitou wielders, read off their own **type-1 CHARACTER templates** in gamedata (`extra['clothing'|'weapons'|'inventory'|'race']`, `ints['armour grade']`), unioned across every definition in load order. Items already resolved to names/kinds, plus `category`/`categoryLabel`/`categoryIndex` (the 11 `CATEGORIES`, in display order — this is the grouping both the page and the bulk `<select>` use, and they must not group the same kits two different ways), `tags` for finer search, advisory `raceNotes` (`{ note, races[] }`), and a `missing[]` of any template this install cannot resolve. A Meitou entry's weapon carries **no `level`**: the grade supplies it |
| GET | `/api/saves` | List save directories, newest first |
| GET | `/api/saves/:name/status` | World summary + squads + characters + inventories. Every inventory row carries `fitWarnings[]` (`{source: 'derived'\|'editorial', text}`) resolved against **that** character — which is how gear already worn by the wrong race is visible without re-equipping it — and each character's `race` carries `armourSlots`/`slotRuleLabel` from the wiki's per-race slot table (null = no known restriction). Each row also carries `colorSid`/`colorName`/`colorHex`/`allowedColors`/`colorsWidened` (TODO.md 3.1), `uniformSid`/`uniformName`/`hasUniform` (TODO.md 3.2 — `hasUniform` decides whether the control is offered, not whether `uniformSid` is empty), and `stolen`/`owner` (TODO.md 3.3). Each character carries a `bounties[]` (TODO.md 3.6, `[]` when none) — `{ index, amount, bountyexp, claim, crimes, factionSid, factionName }`, `factionName` null on a miss, which is rarer than the odd shapes suggest — the literal `defaultEmpireFactionSID` is not stringID-shaped and still resolves, to the United Cities |
...
| GET | `/api/recruits` | "Roll a recruit" catalogue (`services/recruits.js`): 144 entries in 11 groups (soldier, duellist, shadow, ranger, hunter, medic, artisan, trader, explorer, labourer, outcast), at least 8 per group. Carries `group`/`groupLabel`/`groupIndex` (display order), race hint, archetype/sub, tier, `skills` (main ∪ sub) and `tierSpread` (`{ attribute, archRange, otherRange }` — what the tier will actually write, so the Recruits page can show it without resolving archetypes client-side), blurb, `where` resolved against this install's towns, and — on the Meitou wielders — `meitou: true` plus `loadoutId`/`loadoutLabel` pointing at their gear set. Their race/tier/sub come from the game's type-1 template, the weapon class from that weapon's `ints['skill category']` (0 katanas, 1 sabres, 2 blunt, 3 heavy weapons, 4 **hackers**, 8 polearms) |
...
| GET | `/api/colors` | The type-55 colour-scheme catalogue (`services/colorsService.js`), resolved in load order like races/factions: `{ sid, name, color1, color2, hex1, hex2 }[]`, `hexN` being `ints['color N']` (a packed `0xRRGGBB`) rendered as `#RRGGBB` for a swatch. Save-independent |
| POST | `/api/colors/rebuild` | Re-resolve the colour catalogue (after a mod change) |
...
| POST | `/api/saves/:name/platoons/:file/characters` | Add a squad member. `{ name, raceSid, archetype, sub, tier?, provision?, loadoutId?, items? }`. Writes two files — the `.platoon` and `quick.save` — in ONE staged edit, gear included: unless `provision: false`, the new member is minted equipped (armour/weapon(s)/medical kit/food/cats — `services/provisioning.js`), the items landing in that same platoon-file write, never a second mutation. `loadoutId` overrides the archetype-derived default; `items` adds/overrides specific pieces; an unresolvable `loadoutId` is refused before anything is written. See `services/characterFactory.js` for the character records themselves |
| GET | `/api/provisioning/preview` | What `provisionFor()` WOULD give a recruit of this shape — `?archetype=&sub=&tier=&loadoutId=&raceSid=` — without touching any save. Same resolution rules as the write path (a `recruits.js` entry's own `loadoutId` wins, Specialist armour/Catun No.3 weapons, race-aware consumables, a fresh 300-5000 cats roll each call) |
...
| PUT | `/api/saves/:name/platoons/:file/characters/:sid/inventory/:itemSid/uniform` | Set/clear an item's uniform faction tag (`strings.uniform`, TODO.md 3.2); `{ uniformSid }`, `''` clears. **Refused outright** when the record has no `uniform` key at all (type-4/46/102 templates never carry one) — never minted. An unresolved sid is a `warnings[]` entry. Thin wrapper over `updateItem()`. |
| POST | `/api/saves/:name/platoons/:file/characters/:sid/inventory/:itemSid/clear-stolen` | Clear an item's stolen flags (TODO.md 3.3): `ownedbyC/CS/I/S` to 0, `ownedbyTYPE` to 11. Refused if any of those five ints is absent, and refused as a no-op by the mutation gate if the record is already clean. Thin wrapper over `updateItem()`. |
| PUT | `/api/saves/:name/platoons/:file/characters/:sid/inventory/:itemSid` | **Unified per-item edit** — `{ section?, level?, quality?, quantity?, materialSid?, gradeId?, colorSid?, uniformSid?, clearStolen? }`, any combination, in ONE staged edit (one gate pass, one backup). This is what the Gear row's single "Apply" calls; the narrower routes above are wrappers over the same `saveService.updateItem()`. `quantity > 1` is rejected unless the template is stackable. Weapon grade is chosen with `gradeId` and writes `material sid`/`company sid` in lockstep — the **pair** is the grade, `level` is a separate field, but a `gradeId` sent WITHOUT a `level` also writes the ladder row's `rank` into `ints.level` (§3). The receipt's `after.levelFromGrade` says whether it did. Bare `materialSid` still works but is ambiguous (see `/api/gamedata/weapon-grades`) and resolves to the lowest-ranked matching row. `colorSid`/`uniformSid` are `''` to clear; `clearStolen` accepts only `true`. |
...
| PUT | `/api/saves/:name/platoons/:file/characters/:sid/bounties/:n` | Set `ints.amount<n>` on CHAR_STATE (36) — TODO.md 3.6. `{ amount }`, a positive integer only (0 is refused deliberately: the FCS guide warns against it, and a small positive value that expires on its own is the documented safe removal method). Refuses an index whose `amount<n>` isn't already on the record — **there is no route that adds a bounty**, since the whole key family is absent entirely on an unbountied character and this editor never mints one |
| POST | `/api/saves/:name/platoons/:file/characters/:sid/bounties/clear` | Reduce **every** `amount<n>` on a character to the same small positive value (`{ amount? }`, default 1) in ONE staged edit. `bountyexp<n>`/`claim<n>`/`crimes<n>`/`bountyfac<n>` are left untouched. A character with no bounty keys is rejected as a no-op by the mutation gate |
